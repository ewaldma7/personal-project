...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!user_id || !status)  return NextResponse.json('No User ID provided!', { status: 400 });
        const whereFilter : FriendWhereFilterType = {user_id: parseInt(user_id),
...
export async function POST(request: NextRequest) {
    try {
        ...
        const newRequest = await prisma.friend.create({ data: { user_id: body.user_id, friend_id: requestedFriend.user_id, user_requested: true }, include: {friend: true} });
        const newRequest2 = await prisma.friend.create({ data: { user_id: requestedFriend.user_id, friend_id: body.user_id, user_requested: false  } });
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const newRequest = await prisma.friend.update({ data: {status: body.accepted ? 'ACCEPTED' : 'DECLINED'}, where: { user_id_friend_id: {
            user_id: body.user_id,
            friend_id: body.friend_id
          } } });
        const newRequest2 = await prisma.friend.update({ data: {status: body.accepted ? 'ACCEPTED' : 'DECLINED'}, where: { user_id_friend_id: {
